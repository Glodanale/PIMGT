"""Physics Informed Meta Graph Transformer Internal Integration (PIMGT_Internal)"""
import math
...
from models.full_kalman_refinement import FullKalmanRefinementLayer
...
class Decoder(nn.Module):
    def __init__(self, cfgs):
        ...
        # Kalman refinement
        self.kalman = FullKalmanRefinementLayer(
            num_nodes=cfgs['num_nodes'],
            num_features=num_features
        )
        self.dt = 0.1  # NGSIM timestamp resolution (seconds)
    ...
    def forward(self, inputs, targets, c_inputs, c_targets, enc_outputs, transition_matrices, batches_seen):
        ...
        if use_targets is True:
            ...
            B, _, N, C = dec_inputs.shape
            W_t = torch.eye(N, device=dec_inputs.device).unsqueeze(0).repeat(B, 1, 1) * 1e-3
            ...
            for i in range(self.out_len):
                ...
                for decoder_layer in self.layer_stack:
                    out = decoder_layer(
                        ...
                        enc_K,
                        enc_V,
                        c_targets[:, :(i + 1), :, :] if ((not self.noTE) or (not self.noSE)) else None,
                        transition_matrices,
                    )
                    skip += out
                ...
                # Apply Kalman refinement (includes LWR PDE if enabled)
                mu_t, W_t = self.kalman(mu_t, W_t, self.dt)

                outputs.append(mu_t)
                dec_inputs = torch.cat((dec_inputs, mu_t.unsqueeze(1)), dim=1)
...
class PIMGT_Internal(nn.Module):
    def __init__(self, cfgs):
        super(PIMGT_Internal, self).__init__()
        d_model = cfgs['d_model']
