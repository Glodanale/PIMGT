...
def denormalize(tensors, mean, std, type='zscore'):
    ...
    for x in tensors:
        if type == 'zscore':
            ...
            print(f"[DEBUG] Inside denormalize():")
            print(f"  x type: {type(x)}, shape: {x.shape if hasattr(x, 'shape') else 'n/a'}")
            print(f"  std type: {type(std)}, shape: {std.shape if hasattr(std, 'shape') else 'n/a'}")
            print(f"  mean type: {type(mean)}, shape: {mean.shape if hasattr(mean, 'shape') else 'n/a'}")
