...
class StochasticLWRLayer(nn.Module):
    def __init__(self, dx=50.0):
        super().__init__()
        self.dx = dx  # Spacial resolution (distance between nodes)
    ...
    def forward(self, mu):
        ...
        u = mu[:, :, 1]    # [B, N] velocity
        f = rho * u        # [B, N] traffic flux f(p) = rho * u
...
class KalmanRefinementLayer(nn.Module):
    def __init__(self, num_nodes, num_features):
        ...
        self.lwr = StochasticLWRLayer(dx=50.0)  # actual physics component
        self.lam = nn.Parameter(torch.tensor(0.5))  # weight for PDE guidance (lamda)
